...
export default function BoastyProductPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const related = boastyProducts.filter(p => p.slug !== product!.slug && p.category === product!.category).slice(0, 3)

  return (
    <div style={{ fontFamily: "'Helvetica Neue', Helvetica, Arial, sans-serif", background: '#f9f6f0', color: '#1a1a2e', minHeight: '100vh' }}>
      <style>{`
        .bc-pnav { position: sticky; top: 0; z-index: 100; height: 64px; background: rgba(0,58,92,0.97); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: space-between; padding: 0 40px; }
