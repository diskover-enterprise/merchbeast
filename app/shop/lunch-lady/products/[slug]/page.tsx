...
import LunchLadyProductClient from './LunchLadyProductClient'

export const dynamic = 'force-dynamic'

export default async function LunchLadyProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params

  const shop = await prisma.shop.findUnique({ where: { slug: 'lunch-lady' }, select: { id: true } })
  ...
  const product = {
    ...
    sizes: (JSON.parse(raw.sizes || '[]') as string[]).filter(s => s !== '2XL'),
  ...
  const related = relatedRaw.map(p => ({
    ...
    sizes: (JSON.parse(p.sizes || '[]') as string[]).filter(s => s !== '2XL'),
    colors: JSON.parse(p.colors || '[]') as string[],
  }))

  return <>
    {shop && <TrackView shopId={shop.id} productSlug={slug} />}
    <LunchLadyProductClient product={product} related={related} />
  </>
}
