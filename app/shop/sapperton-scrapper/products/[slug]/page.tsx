...
import SappertonProductClient from './SappertonProductClient'

export const revalidate = 60

export default async function SappertonProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params

  const shop = await prisma.shop.findUnique({ where: { slug: 'sapperton-scrapper' }, select: { id: true } })
  ...
  return <>
    ...
    <SappertonProductClient product={product} related={related} />
  </>
}
