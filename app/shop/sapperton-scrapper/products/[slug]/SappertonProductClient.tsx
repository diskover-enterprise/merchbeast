    ...
    <div style={{ fontFamily: "'Barlow Condensed', 'Arial Narrow', sans-serif", background: '#0a0a0a', color: '#f5f5f5', minHeight: '100vh' }}>
      <style>{`
        ...
        @media (max-width: 768px) {
          .ss-pnav { padding: 0 20px; height: 56px; }
      ...
      <div className="ss-pwrap">
        <div className="ss-playout">
            ...
            {product.images.length > 1 && (
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setActiveImg(i)} style={{ width: 72, height: 72, padding: 0, border: i === activeImg ? '2px solid #c8102e' : '2px solid transparent', background: 'transparent', cursor: 'pointer', overflow: 'hidden', flexShrink: 0 }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
          ...
          <div style={{ paddingTop: 8 }}>
            <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: 11, letterSpacing: '0.3em', textTransform: 'uppercase', color: '#c8102e', marginBottom: 10, fontWeight: 600 }}>
              {product.tag || 'Apparel'}
            </p>
            <h1 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 48, fontWeight: 900, fontStyle: 'italic', textTransform: 'uppercase', lineHeight: 1.05, marginBottom: 16, letterSpacing: '-0.01em', color: '#f5f5f5' }}>{product.name}</h1>
            <p style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 36, color: '#c8102e', fontWeight: 900, marginBottom: 24 }}>${product.price}</p>
            <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: 14, lineHeight: 1.7, color: 'rgba(245,245,245,0.55)', marginBottom: 32 }}>{product.description}</p>

            {product.sizes.length > 0 && (
              <div style={{ marginBottom: 28 }}>
                <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: 11, letterSpacing: '0.25em', textTransform: 'uppercase', fontWeight: 600, color: sizeError ? '#c8102e' : 'rgba(245,245,245,0.4)', marginBottom: 12 }}>
                  {sizeError ? 'Please select a size' : 'Size'}
                </p>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {product.sizes.map(s => (
                    <button key={s} onClick={() => setSelectedSize(s)} style={{ minWidth: 52, height: 48, padding: '0 12px', fontSize: 13, fontWeight: 700, fontFamily: "'Barlow Condensed', sans-serif", cursor: 'pointer', border: selectedSize === s ? '2px solid #c8102e' : '2px solid rgba(245,245,245,0.15)', background: selectedSize === s ? '#c8102e' : 'transparent', color: '#f5f5f5', textTransform: 'uppercase', letterSpacing: '0.05em', transition: 'all 0.15s' }}>
                      {s}
                    </button>
                  ))}
                </div>
            ...
            <button onClick={handleAddToCart} style={{ width: '100%', padding: '18px 32px', fontSize: 14, letterSpacing: '0.25em', textTransform: 'uppercase', fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, background: added ? '#8a0a1f' : '#c8102e', color: '#fff', border: 'none', cursor: 'pointer', marginBottom: 12, transition: 'background 0.2s' }}>
            ...
            <p style={{ fontFamily: "'Barlow', sans-serif", fontSize: 11, color: 'rgba(245,245,245,0.25)', marginTop: 20, letterSpacing: '0.1em', textAlign: 'center' }}>Secure checkout powered by Stripe</p>
          </div>
        </div>
              ...
              {related.map(p => (
                <Link key={p.slug} href={`/shop/sapperton-scrapper/products/${p.slug}`} style={{ textDecoration: 'none', color: 'inherit' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <h3 style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 15, fontWeight: 700, fontStyle: 'italic', textTransform: 'uppercase', lineHeight: 1.3, color: '#f5f5f5', flex: 1, marginRight: 8 }}>{p.name}</h3>
                    <span style={{ fontSize: 14, color: '#c8102e', fontWeight: 700, flexShrink: 0 }}>${p.price}</span>
                  </div>
                </Link>
              ))}
            </div>
